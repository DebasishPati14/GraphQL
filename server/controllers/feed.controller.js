...
exports.postFeeds = (req, res, next) => {
  ...
  Post(post)
    ...
    .then((result) => {
      postRecord = result;
      return result.populate('creator', 'name email');
    })
    .then(() => {
    ...
    .then(() => {
      const socketInstance = io.getIO();
      socketInstance.emit('posts', {
        action: 'create',
        post: postRecord,
      });
...
exports.deleteSinglePost = async (req, res, next) => {
  ...
  Post.findById(postId)
    ...
    .then((userDetail) => {
      fs.unlink(
        path.join(__dirname, '..', 'images', postDetail.imageUrl),
        (err) => {
          if (!err) {
      ...
      socketServer.emit('posts', { action: 'delete', postId });
